...
$postgresUrl = $_ENV['POSTGRES_URL'] ?? $_SERVER['POSTGRES_URL'] ?? getenv('POSTGRES_URL') ?? env('POSTGRES_URL');
$databaseUrl = env('DATABASE_URL') ?? $postgresUrl ?? '';
...
$dbUser = $parsedUrl['user'] ?? env('DB_USERNAME', 'forge');
$dbPass = $parsedUrl['pass'] ?? env('DB_PASSWORD', '');
$dbName = isset($parsedUrl['path']) ? ltrim($parsedUrl['path'], '/') : env('DB_DATABASE', 'forge');
...
return [
    ...
    */
    ...
    'connections' => [
        ...
        'pgsql' => [
            'driver' => 'pgsql',
            'url' => str_contains($databaseUrl, 'postgres://') ? null : env('DATABASE_URL'),
            'host' => $dbHost,
            'port' => $dbPort,
            'database' => $dbName,
            'username' => $dbUser,
            ...
            'prefix_indexes' => true,
            'schema' => 'public',
            'sslmode' => str_contains($databaseUrl, 'sslmode=require') ? 'require' : env('DB_SSLMODE', 'prefer'),
        ],
